...
import boxPush3d.utility.Vector3I;
...
public class AABB {
    ...
    public Vector3I getNormal(Vector3d point)
    {
    	byte direction = 0;
    	double lenght = Double.POSITIVE_INFINITY;
    	
    	if(maxX - point.x < lenght) {direction = 0; lenght = maxX - point.x;}
    	if(maxY - point.y < lenght) {direction = 1; lenght = maxY - point.y;}
    	if(maxZ - point.z < lenght) {direction = 2; lenght = maxZ - point.z;}
    	
    	if(point.x - minX < lenght) {direction = 3; lenght = point.x - minX;}
    	if(point.y - minY < lenght) {direction = 4; lenght = point.y - minY;}
    	if(point.z - minZ < lenght) {direction = 5; lenght = point.z - minZ;}
    	
    	if(direction == 0) {return new Vector3I(1, 0, 0);}
    	if(direction == 1) {return new Vector3I(0, 1, 0);}
    	if(direction == 2) {return new Vector3I(0, 0, 1);}
    	
    	if(direction == 3) {return new Vector3I(-1, 0, 0);}
    	if(direction == 4) {return new Vector3I(0, -1, 0);}
    	if(direction == 5) {return new Vector3I(0, 0, -1);}
    	return null;
    }
